...
public class ListingController {
    ...
    // Update the listing status directly
    @PutMapping("/{id}")
    public ResponseEntity<?> updateListingStatus(@PathVariable int id, @RequestBody StatusRequest statusRequest) {
        return listingRepository.findById(id)
                .map(existingListing -> {
                    existingListing.setStatus(Listing.ListingStatus.valueOf(statusRequest.getStatus().toUpperCase()));
